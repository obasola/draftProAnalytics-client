  ...
  <div class="team-schedule-table w-full">
    ...
    >
      ...
      <Column field="scheduleWeek" header="Week" sortable>
        <template #body="{ data }">
          <div
            class="week-badge bg-team-secondary text-team-accent px-2 py-1 rounded text-sm font-medium"
          >
...
const { teams } = storeToRefs(themeStore) as { teams: import('vue').Ref<TeamLite[]> }
...
const validTeamId = computed(() =>
  typeof props.teamId === 'string' ? props.teamId : (props.teamId ?? currentTeam?.value?.id)
)

// Fetch
const fetchPage = async (page = 1, limit = rows.value) => {
  if (!validTeamId.value || !seasonYear.value) return
  await scheduleStore.fetchByTeamSeason(validTeamId.value, seasonYear.value)
}
