...
export const useJobStore = defineStore('jobStore', {
  ...
  actions: {
    ...
    async kickoffEventSync(year: number, seasonType: number, week: number) {
      this.loading = true
      try {
        const res = await api.post('/sync/kickoff/events/by-week', {
          year,
          seasonType,
          week,
        })
        this.lastJob = res.data.job
        return res.data.job
      } finally {
        this.loading = false
      }
