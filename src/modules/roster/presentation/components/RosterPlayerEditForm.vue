<!-- src/modules/roster/presentation/components/rosterPlayer/RosterPlayerEditForm.vue -->
...
import type { UpdateRosterPlayerDto } from '@/modules/roster/application/dto/rosterPlayer.dto'
...
const loadFormData = () => {
  const current = rosterPlayerStore.currentRosterPlayer
  if (current) {
    form.playerName = current.playerName
    form.position = current.position
    form.positionGroup = current.positionGroup
    form.depthChartOrder = current.depthChartOrder
    form.age = current.age
    form.yearsExperience = current.yearsExperience
    form.performanceGrade = current.performanceGrade
    form.isStarter = current.isStarter
    form.contractYearsRemaining = current.contractYearsRemaining
    form.injuryStatus = current.injuryStatus
...
// Watch for changes to current roster player
watch(
  () => rosterPlayerStore.currentRosterPlayer,
  () => {
    loadFormData()
  }
)
...
const onSubmit = async () => {
  ...
  if (!current?.id) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'No roster player selected for editing',
    })
    return
  }

  try {
    const updatePayload: UpdateRosterPlayerDto = {
    ...
    await rosterPlayerStore.update(current.id, updatePayload)
    
    toast.add({
      severity: 'success',
      summary: 'Success',
      detail: 'Roster player updated successfully',
    })
    router.push(`/roster-players/${current.id}?mode=read`)
  } catch (error) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'Failed to update roster player',
    })
  }
}
...
<template>
  <Card class="edit-form">
    <template #title>Edit Roster Player</template>
